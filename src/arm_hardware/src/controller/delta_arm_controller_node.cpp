...
{
  motor_pub_ = create_publisher<arm_msgs::msg::MotorTargets>("arm/motor_targets", 10);
...
{
  pos_pub_ = create_publisher<arm_msgs::msg::ArmPosition>("arm/pos", 10);

  toggle_srv_ = create_service<arm_msgs::srv::TogglePositionStream>(
    "arm/get_pos",
    [this](const std::shared_ptr<arm_msgs::srv::TogglePositionStream::Request> req,
           const std::shared_ptr<arm_msgs::srv::TogglePositionStream::Response> resp) {
...
{
  estop_srv_ = create_service<arm_msgs::srv::EmergencyStop>(
    "arm/emergency_stop",
    [this](const std::shared_ptr<arm_msgs::srv::EmergencyStop::Request> req,
           const std::shared_ptr<arm_msgs::srv::EmergencyStop::Response> resp) {
      if (req->activate) {
...
{
  auto msg = std::make_shared<arm_msgs::msg::MotorTargets>();
...
void DeltaArmControllerNode::publishPosition()
{
  ...
  auto msg = std::make_shared<arm_msgs::msg::ArmPosition>();
  msg->header.stamp = now();
  msg->header.frame_id = feedback_frame_;
