...
MotorDriverNode::MotorDriverNode(const rclcpp::NodeOptions& options)
    : Node("arm_motor_driver", options), feedback_rate_(10.0),
      feedback_frame_("base_link"), online_{0, 0, 0}, error_{0, 0, 0} {
    declareParams();
    ...
    for (size_t i = 0; i < servo_ids.size() && i < 3; ++i)
        ids[i] = static_cast<int>(servo_ids[i]);
    ...
    for (size_t i = 0; i < 3; ++i) {
        ...
        config.install_offset[i] = (i < start_angles.size()) ? static_cast<float>(start_angles[i]) : 0.0f;
    }
    ...
    if (driver_ && auto_init) {
        ...
        if (ok) {
            ...
            for (int i = 0; i < 3; ++i) {
                ...
                if (!online)
                    handleServoFault(i, 2);
            }
        } else {
            RCLCPP_WARN(get_logger(), "motor driver could not sync all servos on %s - is the adapter connected?",
                        port.c_str());
            for (int i = 0; i < 3; ++i)
                handleServoFault(i, 2);
            RCLCPP_WARN(get_logger(), "continuing in feedback-only mode (servos reported offline)");
        }
    }

    if (enable_motion_) {
        target_sub_ = create_subscription<arm::msg::MotorTargets>(
            targets_topic_, 10, std::bind(&MotorDriverNode::onMotorTargets, this, std::placeholders::_1));
    } else {
        RCLCPP_INFO(get_logger(),
                    "motion control DISABLED (enable_motion=false) - "
                    "%s subscription not started",
    ...
    query_srv_ = create_service<arm::srv::MotorParamQuery>(
        ...
        std::bind(&MotorDriverNode::onParamQuery, this, std::placeholders::_1, std::placeholders::_2));
...
void MotorDriverNode::onParamQuery(const std::shared_ptr<arm::srv::MotorParamQuery::Request> request,
                                   const std::shared_ptr<arm::srv::MotorParamQuery::Response> response) {
    response->response_type = request->request_type;
    response->value = 0;
    if (!driver_) {
        response->success = false;
        RCLCPP_WARN_THROTTLE(get_logger(), *get_clock(), 5000, "motor param query ignored - no serial device open");
        return;
    }
    std::lock_guard<std::mutex> lock(bus_mutex_);
    response->success =
        driver_->query(request->servo_id, request->request_type, &response->response_type, &response->value);

    RCLCPP_DEBUG(get_logger(), "query servo=%d type=%d -> response_type=%d value=%d success=%d", request->servo_id,
                 request->request_type, response->response_type, response->value, int(response->success));
}

void MotorDriverNode::handleServoFault(int index, int error_code) {
    error_[index] = static_cast<int8_t>(error_code);
    online_[index] = 0;
    RCLCPP_WARN_THROTTLE(get_logger(), *get_clock(), 5000, "servo %d FAULT code=%d (offline)", index, error_code);
}
...
void MotorDriverNode::publishFeedback() {
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        if (online_[i] == 1) {
            ...
            } else if (++query_fail_streak_[i] < offline_streak_ && last_good_angle_[i] >= 0.0f) {
